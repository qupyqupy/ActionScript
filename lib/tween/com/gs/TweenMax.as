...
DESCRIPTION:
	...
	like pause/resume, timeScale, event listeners, reverse(), restart(), setDestination(), yoyo, loop, rounding, and the ability to jump to any point in the tween using the "progress" property. It also 
	...
	tweening engines out there to choose from, so here's why you might want to consider TweenMax:
		...
		- Feature set - In addition to tweening ANY numeric property of ANY object, TweenMax can tween filters, hex colors, volume, tint, frames, saturation, contrast, hue, colorization, brightness, and even do 
...
SPECIAL PROPERTIES (no plugins required):
	...
	ease:Function - Use any standard easing equation to control the rate of change. For example, gs.easing.Elastic.easeOut. The Default is Regular.easeOut.
...
PLUGINS:
	...
	The following plugins are activated by default in TweenMax (you can easily prevent them from activating, thus saving file size, by commenting out the associated activation lines of code in the constructor):
	  ...
	  bezier:Array - Bezier tweening allows you to tween in a non-linear way. For example, you may want to tween
					   ...
					   through the middle of the tween. Simply pass as many objects in the bezier array as you'd like, one for each "control point" (see documentation on Flash's curveTo() drawing method for more
	  ...
	  hexColors:Object - Although hex colors are technically numbers, if you try to tween them conventionally, you'll notice that they don't tween smoothly. To tween them properly, the red, green, and 
	  ...
	  roundProps:Array - If you'd like the inbetween values in a tween to always get rounded to the nearest integer, use the roundProps
	  					   special property. Just pass in an Array containing the property names that you'd like rounded. For example, if you're tweening the x, y, and alpha properties of mc and you want to round the x and y values (not alpha)
...
EXAMPLES: 
	...
	it has completed and pass in a few parameters to that function (a value of 5 and a reference to the clip_mc), you'd do so like:
...
NOTES / HINTS:
	...
	- If you prefer, instead of using the onCompleteListener, onStartListener, and onUpdateListener special properties, you can set up listeners the typical way, like: 
	...
	- You can kill all tweens of a particular object anytime with the TweenMax.killTweensOf(myObject); 
	  function. If you want to have the tweens forced to completion, pass true as the second parameter, like TweenMax.killTweensOf(myObject, true);
